...
`extensions/chrome`
: Chrome MV3 extension. It injects a content script on supported ydmap booking pages, detects captcha text, normalizes court availability snapshots, and forwards protocol messages to the native host.
...
: Shared Swift package code for message framing, app support storage paths, watch-rule matching, privacy redaction, and native host manifest generation.

`apps/macos/Sources/ZacksBarApp`
: Swift menu bar app skeleton. It owns the status item, menu model, setup/diagnostic entry points, and future native settings UI.
...
3. Availability snapshots emit `availability.updated`.
4. The service worker adds tab context and forwards the message through `chrome.runtime.connectNative`.
5. The native host decodes the Chrome Native Messaging frame and hands the message to Swift core logic.
6. The menu bar app updates status, notifications, and setup health from local state.
